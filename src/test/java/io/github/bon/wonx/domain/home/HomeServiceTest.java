...
public class HomeServiceTest {
        ...
        @Test
        void getBoxOfficeMovies는_boxOfficeRank_오름차순으로_10개_반환한다() {
                movieRepository.deleteAll();

                // boxOfficeRank 10 ~ 1 (뒤집어서 저장)
                for (int i = 10; i >= 1; i--) {
                        movieRepository.save(Movie.builder()
                                        .title("영화 " + i)
                                        .posterUrl("https://poster.example/" + i)
                                        .releaseDate(LocalDate.of(2023, 1, i))
                                        .boxOfficeRank(i)
                                        .viewCount(i * 100)
                                        .build());
                }

                List<BoxOfficeDto> result = homeService.getBoxOfficeMovies();

                assertThat(result).hasSize(10);
                assertThat(result.get(0).getBoxOfficeRank()).isEqualTo(1);
                assertThat(result.get(0).getTitle()).isEqualTo("영화 1");
                assertThat(result.get(9).getBoxOfficeRank()).isEqualTo(10);
        }

        @Test
        void getTotalReviewCount는_누적리뷰_전체개수를_반환한다() {
                hotTalkRepository.deleteAll();
                movieRepository.deleteAll();
                ...
                // 리뷰 더미 3개
                hotTalkRepository.saveAll(List.of(
                                HotTalk.builder().content("후기1").viewCount(10).createdAt(LocalDateTime.now())
                                                .movie(movie)
                                                .build(),
                                HotTalk.builder().content("후기2").viewCount(20).createdAt(LocalDateTime.now())
                                                .movie(movie)
                                                .build(),
                                HotTalk.builder().content("후기3").viewCount(30).createdAt(LocalDateTime.now())
                                                .movie(movie)
                                                .build()));
